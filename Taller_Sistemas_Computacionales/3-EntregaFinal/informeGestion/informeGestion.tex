\documentclass[12pt,letterpaper]{article}
\usepackage[utf8]{inputenc} %Codificacion del texto (ISO Latin1 encoding)
\usepackage{graphicx} %Permite exportar imagenes en formato eps
\usepackage{fancyhdr} %Permite acomodar a tu gusto la parte de arriba y
\usepackage[hmargin=1in,vmargin=1in, lmargin=1in, rmargin=1in]{geometry}
\usepackage[final]{pdfpages}
\usepackage{amssymb}
\usepackage{amsmath}


\pagestyle{fancyplain}

\lhead{\it Segundo Semestre del 2009} %Parte superior izquierda
\rhead{\bf \it Informe de Gestión} %Parte superior derecha
\lfoot{} %Parte inferior izquierda. \thepage indica
% el numero de pagina
\cfoot{} %Parte inferior central
\rfoot{\bf \thepage} %Parte inferior derecha
\renewcommand{\footrulewidth}{0.4pt} %Linea de separacion inferior

\title{Informe de Gestión proyecto \\``Autenticación JAAS para máquinas JBoss''}

\author{
Esteban F. Bombal,
Rodrigo G. Fernández,
Cristián D. Maureira,\\
Ignacio J. Villacura,
Gabriel A. Zamora
}

\newpage
\begin{document}
\bibliographystyle{plain}
\maketitle\thispagestyle{empty}

\section{Organización del grupo de trabajo}
\label{target:reprogramacion}
\input{src/1-organizacion}

\section{Planificación}
\label{target:reprogramacion}
\input{src/2-planificacion}

\section{Autoevaluación global del grupo}
\label{target:reprogramacion}
\input{src/3-autoevaluacion}

\section{Sugerencias y Recomendaciones}
\label{target:reprogramacion}
\input{src/4-sugerencias}

\vfill \hfill EB/RF/CM/IV/GZ/DI/UTFSM/2009
\end{document}
